...
«IF game.grid.size !== null»        return new Dimension(10*«game.grid.size.x»+50, 10*«game.grid.size.y»+100);
«ELSE»	    return new Dimension(10*80+50, 10*60+100);
«ENDIF»
    }
}''')

	// Also generate GameOfLife, to make the package executable
		genGame(fsa);
	}
	...
	def static addMarkings(Game game) '''
			...
			«FOR m: game.grid.marks» 
				«IF m.poi !== null»	     survivingCells.add(new Point(«m.poi.x», «m.poi.y»));
				«ELSE»
					«IF m.pat.name !== null»        for (Point p : «m.pat.name») {
						        survivingCells.add(new Point(p.x+«m.pat.loc.x», p.y+«m.pat.loc.y»));	
					        }
	...
	def genGame(IFileSystemAccess2 fsa) {
		fsa.generateFile('GameOfLife.java', '''package GameOfLife;
		...
		public class GameOfLife extends JFrame implements ActionListener {
		    private static final Dimension DEFAULT_WINDOW_SIZE = RulesOfLife.getGridSize();
		    ...
		    private class GameBoard extends JPanel implements ComponentListener, MouseListener, MouseMotionListener, Runnable {
		        ...
		        public void resetBoard() {
		        	point.clear();
		        	ArrayList<Point> init = new ArrayList<Point>(0);
		            RulesOfLife.addMarkings(init);
		            point.addAll(init);
		        }
